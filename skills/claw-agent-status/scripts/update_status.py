#!/usr/bin/env python3
"""Update a Claw OS manual agent status."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Update a Claw OS agent status.")
    ...
    parser.add_argument(
        "--token",
        default=os.environ.get("CLAW_OS_AGENT_TOKEN", ""),
        help="Manual agent token. Defaults to CLAW_OS_AGENT_TOKEN.",
    )
    parser.add_argument(
        "--url",
        default=os.environ.get("CLAW_OS_AGENT_STATUS_URL", DEFAULT_URL),
        help=f"Agent status API URL. Defaults to CLAW_OS_AGENT_STATUS_URL or {DEFAULT_URL}.",
    )
...
def main() -> int:
    ...
    if args.state not in ACCEPTED_STATES:
        ...
        return 2
    if not args.token:
        print("Missing agent token. Set CLAW_OS_AGENT_TOKEN or pass --token.", file=sys.stderr)
        return 2
